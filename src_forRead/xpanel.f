...
C***********************************************************************
      ...
      INCLUDE 'XFOIL.INC'
C---------------------------------------
C     panel angle defined as the vector angle of the normal vector which is rotated from tangent vector CCW by 90 degrees
C---------------------------------------
C
...
C
      ...
      DO 10 I=1, N
c------ rotating tangent vector CW by 90 degrees?
        SX =  YN(I)
        SY = -XN(I)
...
C
      DO 10 JO=1, N
        JP = JO+1
c------ JO and JP are indices for panel nodes 1 and 2
C
        JM = JO-1
        JQ = JP+1
C
...
C
        DSIO = 1.0 / DSO
C
        APAN = APANEL(JO) ! get panel angle
C
c------ (XI,YI) is collocation/control point
        RX1 = XI - X(JO)
        RY1 = YI - Y(JO)
        RX2 = XI - X(JP)
        RY2 = YI - Y(JP)
C
c------ tangent vector along sRef
        SX = (X(JP) - X(JO)) * DSIO
        SY = (Y(JP) - Y(JO)) * DSIO
C
        X1 = SX*RX1 + SY*RY1  ! Xbar1
        X2 = SX*RX2 + SY*RY2  ! Xbar2
        YY = SX*RY1 - SY*RX1  ! Ybar
C
        RS1 = RX1*RX1 + RY1*RY1
        RS2 = RX2*RX2 + RY2*RY2
C
